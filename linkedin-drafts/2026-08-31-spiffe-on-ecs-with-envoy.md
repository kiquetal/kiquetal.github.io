...
---
## ✅ READY TO POST — Single bilingual post (2,334 chars, under LinkedIn's 3,000 limit)

🇬🇧 How do you get zero-trust mTLS on ECS Fargate when there's no EC2 instance to attest against? 🔐

I love Istio — but I wanted to understand what happens under the hood. So I built Proteus: an mTLS mesh for Amazon ECS from scratch, using SPIFFE/SPIRE + Envoy.
...
Identity is established in two layers: node attestation (which task SPIRE trusts) and workload attestation + admission (which service identity that task may hold).

The result is deny-by-default — "dark until admitted." No workload gets an SVID until it's explicitly admitted. Then the SVID flows over SDS, Envoy completes the mTLS handshake, and service-a → service-b works — with short-lived X.509 certs and zero long-lived secrets in the app.

👉 https://kiquetal.github.io/blog/2026-08-31-spiffe-on-ecs-with-envoy

—

🇪🇸 ¿Cómo lograr mTLS zero-trust en ECS Fargate cuando no hay una instancia EC2 contra la cual verificar la identidad? 🔐

Me gusta Istio — pero quería entender qué pasa por debajo. Así que construí Proteus: una malla mTLS para Amazon ECS desde cero, con SPIFFE/SPIRE + Envoy.

La parte difícil: en Fargate no hay host, así que el verificador de nodo aws_iid no aplica. Escribí un plugin propio de SPIRE que lee el endpoint de metadata de la tarea ECS — y lo verifica del lado del servidor vía la API de ECS (DescribeTasks) contra una lista de roles IAM permitidos. Una tarea puede afirmar un ARN, pero no puede falsificarlo.

La identidad se establece en dos capas: verificación de nodo (en qué tarea confía SPIRE) y verificación de workload + admisión (qué identidad de servicio puede tener esa tarea).

El resultado es denegar-por-defecto — "oscuro hasta ser admitido." Ningún workload obtiene un SVID hasta ser admitido. Luego el SVID fluye vía SDS, Envoy completa el handshake mTLS, y service-a → service-b funciona — con certificados X.509 de corta duración y cero secretos de larga duración en la app.

👉 https://kiquetal.github.io/blog/2026-08-31-spiffe-on-ecs-with-envoy

#SPIFFE #SPIRE #ECS #Envoy #mTLS #ZeroTrust #AWS #CloudSecurity #Fargate #DevSecOps
...
La parte difícil: en Fargate no hay host, así que el verificador de nodo habitual `aws_iid` no aplica. Escribí un plugin propio de SPIRE que lee el endpoint de metadata de la tarea ECS — y luego lo verifica **del lado del servidor** vía la API de ECS (`DescribeTasks`) contra una lista de roles IAM permitidos. Una tarea puede *afirmar* un ARN, pero no puede falsificarlo.

La identidad se establece en dos capas:

1️⃣ **Verificación de nodo** — SPIRE decide en qué *tarea* confía.
2️⃣ **Verificación de workload + admisión** — un admission controller decide qué *identidad de servicio* puede tener esa tarea.
...
Si ya hiciste SPIFFE/SPIRE en Kubernetes, ECS te obliga a repensar varias suposiciones (verificación de nodo vs. workload, compartir un socket Unix entre contenedores de una tarea, y dónde vive el agente SPIRE). Documenté la arquitectura, el razonamiento y el recorrido oscuro-vs-activo con logs reales.
